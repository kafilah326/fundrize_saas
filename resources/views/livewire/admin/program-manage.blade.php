<div>
    ...
    <div class="space-y-6">
        ...
        <div
            class="bg-white rounded-2xl p-6 shadow-soft border border-gray-100 flex flex-col md:flex-row gap-6 items-start">
            <div class="flex-shrink-0">
                <img src="{{ $program->image }}"
                    class="w-32 h-32 rounded-xl object-cover shadow-md border border-gray-100"
                    alt="{{ $program->title }}">
            </div>
            <div class="flex-grow">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2 mt-2">
                            @foreach ($program->categories as $category)
                                <span
                                    class="px-2 py-1 bg-blue-50 text-blue-600 text-xs rounded-lg font-medium border border-blue-100">{{ $category->name }}</span>
                            @endforeach
                            @foreach ($program->akads as $akad)
                                <span
                                    class="px-2 py-1 bg-emerald-50 text-emerald-600 text-xs rounded-lg font-medium border border-emerald-100">{{ $akad->name }}</span>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
                    ...
                    <div class="bg-orange-50 rounded-xl p-3 border border-orange-100">
                        ...
                        <p class="text-lg font-bold text-orange-700">Rp
            ...
            <div x-show="$wire.activeTab === 'updates'" class="p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <button wire:click="createUpdate"
                            class="px-4 py-2 bg-primary text-white rounded-xl text-sm font-medium hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all hover:-translate-y-0.5">
                ...
                <div class="space-y-4">
                    @forelse($updates as $update)
                        <div
                            class="border border-gray-100 rounded-xl p-4 hover:bg-gray-50 transition-colors flex flex-col sm:flex-row gap-4">
                            ...
                            <div class="flex-grow">
                                <div class="flex justify-between items-start">
                                    ...
                                    <div class="flex space-x-2">
                                        ...
                                        <button wire:click="confirmDeleteUpdate({{ $update->id }})"
                    ...
                    @forelse($distributions as $dist)
                        <div
                            ...
                            <div class="flex-grow">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <h4 class="font-bold text-gray-900">Rp
                                            {{ number_format($dist->amount_distributed, 0, ',', '.') }}</h4>
                                        <p class="text-xs text-gray-500 mb-2">
                                            <i class="fa-regular fa-calendar-days mr-1"></i>
                                            {{ $dist->documentation_date->format('d M Y') }}
                                        </p>
                                    </div>
                                    <div class="flex space-x-2">
    ...
    <!-- Update Modal -->
    <div x-data="{ show: $wire.entangle('showUpdateModal') }" x-show="show" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
                ...
                <form wire:submit.prevent="storeUpdate">
                    <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                        <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">
                            {{ $updateId ? 'Edit Kabar' : 'Buat Kabar Baru' }}</h3>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Judul Kabar</label>
                                <input type="text" wire:model="updateTitle"
                                    class="block w-full mt-1 border-gray-300 rounded-xl shadow-sm focus:ring-primary focus:border-primary py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                @error('updateTitle')
                            ...
                            <div>
                                ...
                                <input type="date" wire:model="updatePublishedAt"
                                    class="block w-full mt-1 border-gray-300 rounded-xl shadow-sm focus:ring-primary focus:border-primary py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                @error('updatePublishedAt')
                                    <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
                            <div>
                    ...
                    <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="submit"
                            class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:ml-3 sm:w-auto sm:text-sm transition-colors">
                            Simpan
                        </button>
    ...
    <!-- Distribution Modal -->
    <div x-data="{ show: $wire.entangle('showDistributionModal') }" x-show="show" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 transition-opacity bg-gray-900/60 backdrop-blur-sm" @click="show = false"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div
                ...
                <form wire:submit.prevent="storeDistribution">
                    <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                        <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">
                            {{ $distributionId ? 'Edit Penyaluran' : 'Input Penyaluran Baru' }}</h3>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Jumlah Disalurkan (Rp)</label>
                                <input type="number" wire:model="distributionAmount"
                                    class="block w-full mt-1 border-gray-300 rounded-xl shadow-sm focus:ring-primary focus:border-primary py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                            ...
                            <div>
                                ...
                                <input type="date" wire:model="distributionDate"
                                    class="block w-full mt-1 border-gray-300 rounded-xl shadow-sm focus:ring-primary focus:border-primary py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                    ...
                    <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button type="button" wire:click="$set('showDistributionModal', false)"
                            class="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm transition-colors">
                            Batal
                        </button>
                    </div>
    ...
    <div x-data="{ show: $wire.entangle('confirmingUpdateDeletion') }" x-show="show" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 transition-opacity bg-gray-900/60 backdrop-blur-sm" @click="show = false"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div
                ...
                <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button wire:click="deleteUpdate" type="button"
                    ...
                    <button wire:click="$set('confirmingUpdateDeletion', false)" type="button"
                        class="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm transition-colors">
                        Batal
                    </button>
                </div>
    ...
    <div x-data="{ show: $wire.entangle('confirmingDistributionDeletion') }" x-show="show" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 transition-opacity bg-gray-900/60 backdrop-blur-sm" @click="show = false"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen">&#8203;</span>
            <div
                ...
                <div class="px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">Apakah Anda yakin ingin menghapus data penyaluran ini?
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button wire:click="deleteDistribution" type="button"
                        class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm transition-colors">
                        Hapus
                    </button>
                    <button wire:click="$set('confirmingDistributionDeletion', false)" type="button"
                        class="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm transition-colors">
                        Batal
                    </button>
                </div>
