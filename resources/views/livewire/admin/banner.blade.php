        ...
        <div class="p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                <div class="relative w-full sm:w-1/3 group">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari banner..." class="pl-10 w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary focus:ring focus:ring-primary/20 transition-all duration-200 py-2.5 px-4 text-base">
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50/80">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Halaman</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Link</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Periode</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Urutan</th>
                            ...
                            <tr class="hover:bg-orange-50/30 transition-colors duration-150 group">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-md 
                                        {{ $banner->placement === 'home' ? 'bg-blue-100 text-blue-800' : 
                                           ($banner->placement === 'qurban' ? 'bg-orange-100 text-orange-800' : 
                                           ($banner->placement === 'qurban_tabungan' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800')) }}">
                                        {{ $banner->placement === 'qurban' ? 'Qurban' : ($banner->placement === 'qurban_tabungan' ? 'Tabungan Qurban' : 'Home') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <a href="{{ $banner->link_url }}" target="_blank" class="text-blue-500 hover:text-blue-700 hover:underline truncate w-32 block flex items-center">
                                        <i class="fa-solid fa-up-right-from-square mr-1 text-xs"></i> Link
                                    </a>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="flex flex-col">
                                        <span class="text-xs text-gray-400">Mulai: {{ $banner->start_date ? $banner->start_date->format('d M Y') : '-' }}</span>
                                        <span class="text-xs text-gray-400">Selesai: {{ $banner->end_date ? $banner->end_date->format('d M Y') : '-' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button wire:click="toggleStatus({{ $banner->id }})" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full transition-colors duration-200 {{ $banner->is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200' }}">
                                        {{ $banner->is_active ? 'Active' : 'Inactive' }}
                                    </button>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <button wire:click="edit({{ $banner->id }})" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                            <i class="fa-solid fa-pen-to-square"></i>
                                        </button>
                                        <button wire:click="confirmDelete({{ $banner->id }})" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm text-gray-500 mt-1">Tambahkan banner untuk promosi di halaman utama.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
                    ...
                    <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                        ...
                        <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-6">
                                <label for="placement" class="block text-sm font-semibold text-gray-700 mb-1">Halaman</label>
                                <select wire:model="placement" id="placement" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 transition-shadow py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                ...
                                @error('placement') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            <!-- Title -->
                            <div class="sm:col-span-6">
                                <label for="title" class="block text-sm font-semibold text-gray-700 mb-1">Judul Banner</label>
                                <input wire:model="title" type="text" id="title" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 transition-shadow py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                @error('title') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            <!-- Image -->
                            <div class="sm:col-span-6">
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Gambar Banner (16:9)</label>
                            ...
                            <div class="sm:col-span-4">
                                <label for="link_url" class="block text-sm font-semibold text-gray-700 mb-1">Link URL</label>
                                <div class="relative rounded-xl shadow-sm">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <i class="fa-solid fa-link text-gray-400"></i>
                                    </div>
                                    <input wire:model="link_url" type="url" id="link_url" placeholder="https://" class="pl-10 block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                </div>
                                @error('link_url') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            <!-- CTA Text -->
                            <div class="sm:col-span-2">
                                <label for="cta_text" class="block text-sm font-semibold text-gray-700 mb-1">Teks Tombol</label>
                                <input wire:model="cta_text" type="text" id="cta_text" placeholder="Donasi Sekarang" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                @error('cta_text') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>

                            <!-- Dates -->
                            <div class="sm:col-span-3">
                                <label for="start_date" class="block text-sm font-semibold text-gray-700 mb-1">Mulai Tayang</label>
                                <input wire:model="start_date" type="date" id="start_date" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                @error('start_date') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                            </div>
                            <div class="sm:col-span-3">
                                <label for="end_date" class="block text-sm font-semibold text-gray-700 mb-1">Selesai Tayang</label>
                            ...
                            <!-- Priority & Checkbox -->
                            <div class="sm:col-span-3">
                                <label for="priority" class="block text-sm font-semibold text-gray-700 mb-1">Urutan Prioritas</label>
                                <input wire:model="priority" type="number" id="priority" min="0" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
