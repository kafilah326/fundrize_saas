                    ...
                    @forelse($followups as $followup)
                        <tr class="hover:bg-orange-50/30 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col space-y-1">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium max-w-max
                                        {{ $followup->type === 'donasi' ? 'bg-blue-100 text-blue-800' : 
                                           ($followup->type === 'qurban' ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800') }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $followup->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $followup->is_active ? 'Aktif' : 'Non-aktif' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button wire:click="edit({{ $followup->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3 transition-colors duration-200">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <button wire:click="confirmDelete({{ $followup->id }})" class="text-red-600 hover:text-red-900 transition-colors duration-200">
            ...
            <div x-show="show"
                ...
                <form wire:submit.prevent="{{ $isEditing ? 'update' : 'store' }}">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                                    ...
                                    <div>
                                        ...
                                        <input wire:model="name" type="text" class="w-full rounded-xl border-gray-200 bg-gray-50/50 shadow-sm focus:bg-white focus:border-primary focus:ring focus:ring-primary/20 transition-all px-4 py-2.5" placeholder="Contoh: Followup Donasi Harian">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                                    ...
                                    <div>
                                        ...
                                        <select wire:model.live="type" class="w-full rounded-xl border-gray-200 bg-gray-50/50 shadow-sm focus:bg-white focus:border-primary focus:ring focus:ring-primary/20 transition-all px-4 py-2.5">
                                    ...
                                    <!-- Sequence -->
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Urutan Followup</label>
                                        <select wire:model="followup_sequence" class="w-full rounded-xl border-gray-200 bg-gray-50/50 shadow-sm focus:bg-white focus:border-primary focus:ring focus:ring-primary/20 transition-all px-4 py-2.5">
                                        ...
                                        @error('followup_sequence') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                    </div>
                                </div>

                                <!-- Dynamic Parameters -->
                                <div class="mb-2">
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">Parameter Tersedia (Klik untuk menyisipkan)</label>
                                    <div class="flex flex-wrap gap-2 p-3 bg-gray-50 rounded-xl border border-gray-200">
                                        @php
                                            $commonParams = ['{{nama}}', '{{tanggal}}'];
                                            $params = [];
                                            if($type == 'donasi') {
                                                $params = array_merge($commonParams, ['{{program}}', '{{nilai_donasi}}', '{{link_donasi}}', '{{link_pembayaran}}']);
                                            } elseif($type == 'qurban') {
                                                $params = array_merge($commonParams, ['{{jenis_hewan}}', '{{tipe_qurban}}', '{{harga}}', '{{link_pembayaran}}']);
                                            } elseif($type == 'tabungan_qurban') {
                                                $params = array_merge($commonParams, ['{{target_tabungan}}', '{{saldo_saat_ini}}', '{{sisa_pembayaran}}', '{{link_topup}}', '{{link_pembayaran}}']);
                                            }
                                        @endphp

                                        @foreach($params as $param)
                                            <button type="button" wire:click="insertParameter('{{ $param }}')" 
                                ...
                                <div>
                                    ...
                                    <textarea wire:model="content" rows="6" class="w-full rounded-xl border-gray-200 bg-gray-50/50 shadow-sm focus:bg-white focus:border-primary focus:ring focus:ring-primary/20 transition-all font-mono text-sm px-4 py-3" placeholder="Tulis pesan followup disini..."></textarea>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-2xl">
                        <button type="submit" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:ml-3 sm:w-auto sm:text-sm transition-colors duration-200">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Apakah Anda yakin ingin menghapus template followup ini? Tindakan ini tidak dapat dibatalkan.
                                </p>
                            </div>
                        </div>
                    </div>
