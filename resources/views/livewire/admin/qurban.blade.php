...
<div class="space-y-6">
        ...
        <div class="p-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                <div class="relative w-full sm:w-1/3 group">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari data..." class="pl-10 w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary focus:ring focus:ring-primary/20 transition-all duration-200 py-2.5 px-4 text-base">
                </div>
                
                @if($activeTab === 'animals')
                <button wire:click="createAnimal" class="w-full sm:w-auto bg-primary hover:bg-primary-hover text-white font-semibold py-2.5 px-5 rounded-xl inline-flex items-center justify-center transition-all duration-200 shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:-translate-y-0.5">
                    <i class="fa-solid fa-plus mr-2"></i> Tambah {{ $animalType === 'tabungan' ? 'Target Tabungan' : 'Hewan' }}
                </button>
                @elseif($activeTab === 'orders' || $activeTab === 'savings')
                <button wire:click="openExportModal" class="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 px-5 rounded-xl inline-flex items-center justify-center transition-all duration-200 shadow-lg shadow-green-600/30 hover:shadow-green-600/50 hover:-translate-y-0.5 whitespace-nowrap">
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-orange-50/30 transition-colors duration-150 group">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">
                                        <span class="px-2 py-1 bg-gray-100 rounded-md text-xs font-medium">{{ $animal->category }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Rp {{ number_format($animal->price, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span class="{{ $animal->stock < 5 ? 'text-red-500 font-bold' : '' }}">{{ $animal->stock }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <button wire:click="toggleAnimalStatus({{ $animal->id }})" class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full transition-colors duration-200 {{ $animal->is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-red-100 text-red-700 hover:bg-red-200' }}">
                                            {{ $animal->is_active ? 'Active' : 'Inactive' }}
                                        </button>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <button wire:click="editAnimal({{ $animal->id }})" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </button>
                                            <button wire:click="deleteAnimal({{ $animal->id }})" onclick="return confirm('Yakin hapus hewan ini?') || event.stopImmediatePropagation()" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus">
                                ...
                                @empty
                                <tr><td colspan="7" class="px-6 py-12 text-center text-gray-500">Tidak ada data hewan.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50/80">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Donatur</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-orange-50/30 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{{ $order->transaction_id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-semibold text-gray-900">{{ $order->donor_name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                        <span class="font-medium">{{ $order->animal->name ?? '-' }}</span>
                                        <span class="text-xs text-gray-400 ml-1">({{ $order->hijri_year }})</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $order->qurban_name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @empty
                                <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">Tidak ada pesanan qurban.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    @elseif($activeTab === 'savings')
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-orange-50/30 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{{ $saving->donor_name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">{{ str_replace('-', ' ', $saving->target_animal_type) }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-800">Rp {{ number_format($saving->saved_amount, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div class="flex items-center">
                                            <span class="mr-2 w-8 text-right font-medium text-xs">{{ $saving->progress }}%</span>
                                            <div class="w-24 bg-gray-100 rounded-full h-1.5 overflow-hidden">
                                                <div class="bg-gradient-to-r from-primary to-secondary h-1.5 rounded-full" style="width: {{ $saving->progress }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @empty
                                <tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">Tidak ada data tabungan.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                        ...
                        <div class="p-6">
                            <form wire:submit.prevent="saveTabunganContent">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <!-- Left Column -->
                                    <div class="space-y-6">
                                        <h3 class="text-lg font-bold text-gray-900 border-b pb-2">Header & Deskripsi</h3>
                                        
                                        <div>
                                            <label class="block text-sm font-semibold text-gray-700 mb-1">Judul Halaman</label>
                                            <input wire:model="contentTitle" type="text" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                            @error('contentTitle') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                        </div>

                                        <div>
                                            <label class="block text-sm font-semibold text-gray-700 mb-1">Subtitle</label>
                                            <input wire:model="contentSubtitle" type="text" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                            @error('contentSubtitle') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                        </div>
                                        ...
                                        <div wire:ignore>
                                            ...
                                            @error('contentDescription') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                        </div>

                                        <div class="space-y-3">
                                            <div class="flex justify-between items-center">
                                                <label class="block text-sm font-semibold text-gray-700">Keunggulan Program</label>
                                                <button type="button" wire:click="addBenefit" class="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-2 py-1 rounded-lg font-medium transition-colors">
                                                    <i class="fa-solid fa-plus mr-1"></i> Tambah
                                                </button>
                                            </div>
                                            @foreach($contentBenefits as $index => $benefit)
                                            <div class="flex gap-2">
                                                <input wire:model="contentBenefits.{{ $index }}" type="text" class="block w-full rounded-lg border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2 px-3 text-sm bg-gray-50 focus:bg-white transition-colors">
                                                <button type="button" wire:click="removeBenefit({{ $index }})" class="p-2 text-red-500 hover:bg-red-50 rounded-lg">
                                                    <i class="fa-solid fa-trash"></i>
                                                </button>
                                            </div>
                                            @endforeach
                                            @error('contentBenefits.*') <span class="text-red-500 text-xs block">{{ $message }}</span> @enderror
                                        </div>
                                    </div>

                                    <!-- Right Column -->
                                    <div class="space-y-6">
                                        <h3 class="text-lg font-bold text-gray-900 border-b pb-2">Informasi Akad & Syarat</h3>

                                        <div>
                                            <label class="block text-sm font-semibold text-gray-700 mb-1">Judul Akad</label>
                                            <input wire:model="contentAkadTitle" type="text" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                        </div>

                                        <div>
                                            <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi Akad</label>
                                            <textarea wire:model="contentAkadDescription" rows="2" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors"></textarea>
                                        </div>

                                        <div class="space-y-3">
                                            <div class="flex justify-between items-center">
                                                <label class="block text-sm font-semibold text-gray-700">Syarat & Ketentuan (Modal)</label>
                                                <button type="button" wire:click="addTerm" class="text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 px-2 py-1 rounded-lg font-medium transition-colors">
                                                    <i class="fa-solid fa-plus mr-1"></i> Tambah
                                                </button>
                                            </div>
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="p-3 bg-gray-50 rounded-xl border border-gray-200 relative group">
                                                    <button type="button" wire:click="removeTerm({{ $index }})" class="absolute top-2 right-2 text-gray-400 hover:text-red-500 transition-colors">
                                                        <i class="fa-solid fa-xmark"></i>
                                                    </button>
                                                    <div class="space-y-2">
                                                        <input wire:model="contentTerms.{{ $index }}.title" type="text" placeholder="Judul Syarat" class="block w-full rounded-lg border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-1.5 px-3 text-sm font-semibold bg-white focus:bg-white transition-colors">
                                                        <textarea wire:model="contentTerms.{{ $index }}.description" rows="2" placeholder="Penjelasan..." class="block w-full rounded-lg border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-1.5 px-3 text-xs bg-white focus:bg-white transition-colors"></textarea>
                                                    </div>
                                                </div>
                                                @endforeach
                                            </div>
                                            @error('contentTerms.*.title') <span class="text-red-500 text-xs block">{{ $message }}</span> @enderror
                                            @error('contentTerms.*.description') <span class="text-red-500 text-xs block">{{ $message }}</span> @enderror
                                        </div>
                                    </div>
                                </div>
    ...
    <!-- Animal Modal -->
    <div x-data="{ show: $wire.entangle('isAnimalModalOpen') }"
         x-show="show"
                ...
                <form wire:submit.prevent="saveAnimal">
                    <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                        <div class="flex justify-between items-center mb-6">
                            <div>
                                <h3 class="text-xl font-bold text-gray-900">{{ $animalId ? 'Edit' : 'Tambah' }} {{ $type === 'tabungan' ? 'Target Tabungan' : 'Hewan Qurban' }}</h3>
                                <span class="inline-flex items-center px-2 py-0.5 mt-1 rounded-md text-xs font-medium {{ $type === 'tabungan' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700' }}">
                                    {{ $type === 'tabungan' ? 'Qurban Tabungan' : 'Qurban Langsung' }}
                                </span>
                            </div>
                            <button type="button" wire:click="closeAnimalModal" class="text-gray-400 hover:text-gray-500"><i class="fa-solid fa-xmark text-xl"></i></button>
                        </div>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select wire:model="category" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                ...
                                <div>
                                    ...
                                    <input wire:model="weight" type="text" placeholder="ex: 25-30 kg" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input wire:model="price" type="number" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                                    @error('price') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Stok</label>
                                    <input wire:model="stock" type="number" class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base bg-gray-50 focus:bg-white transition-colors">
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi <span class="text-gray-400 text-xs">(Opsional)</span></label>
                                <textarea wire:model="description" rows="2" placeholder="Keterangan tambahan..." class="block w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-base resize-none bg-gray-50 focus:bg-white transition-colors"></textarea>
    ...
    <!-- Order Detail Modal -->
    <div x-data="{ show: $wire.entangle('isOrderModalOpen') }"
         x-show="show"
         x-transition:enter="transition ease-out duration-300"
         class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full border border-gray-100">
                ...
                <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button type="button" wire:click="closeOrderModal" class="text-gray-400 hover:text-gray-500"><i class="fa-solid fa-xmark text-xl"></i></button>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-lg font-mono font-bold text-gray-800">{{ $selectedOrder->transaction_id }}</p>
                            </div>
                            <span class="px-3 py-1 rounded-lg text-sm font-bold {{ $selectedOrder->status == 'paid' ? 'bg-green-100 text-green-700' : ($selectedOrder->status == 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">
                        ...
                        <div class="border-t border-gray-100 pt-4">
                            <div class="flex items-center mb-2">
                                <div class="w-8 h-8 rounded-full bg-green-100 text-green-600 flex items-center justify-center mr-3"><i class="fa-solid fa-cow"></i></div>
                                <div>
                            ...
                            <div class="ml-11 text-sm text-gray-600 space-y-1">
                                <p>Sembelih: <span class="font-medium text-gray-900">{{ ucfirst($selectedOrder->slaughter_method) }}</span></p>
                                <p>Pengiriman: <span class="font-medium text-gray-900">{{ ucfirst($selectedOrder->delivery_method) }}</span></p>
                                @if($selectedOrder->address)
                                <p class="bg-gray-50 p-2 rounded text-xs mt-1">{{ $selectedOrder->address }}, {{ $selectedOrder->city }} {{ $selectedOrder->postal_code }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="border-t border-gray-100 pt-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="font-medium capitalize">{{ str_replace('_', ' ', $selectedOrder->payment_method ?? '-') }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Tipe Pembayaran</p>
                                    <p class="font-medium capitalize">{{ str_replace('_', ' ', $selectedOrder->payment->payment_type ?? '-') }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500">Status Pembayaran</p>
                                    <span class="px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full {{ ($selectedOrder->payment->status ?? '') == 'paid' ? 'bg-green-100 text-green-700' : (($selectedOrder->payment->status ?? '') == 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">
                                ...
                                <div>
                                    ...
                                    <p class="font-medium text-sm">{{ $selectedOrder->payment->paid_at->format('d M Y H:i') }}</p>
                                </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="border-t border-gray-100 pt-4 flex justify-between items-center">
                            ...
                            <span class="text-xl font-bold text-primary">Rp {{ number_format($selectedOrder->amount, 0, ',', '.') }}</span>
                        </div>
                        ...
                        <div class="border-t border-gray-100 pt-4">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                                ...
                                <div class="relative group rounded-lg overflow-hidden bg-gray-100 aspect-video border border-gray-200">
                                    ...
                                    @else
                                        ...
                                        <div class="absolute inset-0 flex items-center justify-center pointer-events-none">
                                            <i class="fa-solid fa-play-circle text-white text-3xl opacity-80"></i>
                                        </div>
                                    @endif
                                    ...
                                    <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-10">
                                        <button wire:click="deleteDocumentation({{ $doc->id }}, 'order')" 
                                        ...
                                        <a href="{{ Storage::url($doc->file_path) }}" target="_blank" class="ml-2 p-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 shadow-sm transform hover:scale-110 transition-all">
                                            <i class="fa-solid fa-expand text-xs"></i>
                                        </a>
                                    </div>
                                    
                                    @if($doc->caption)
                                    <div class="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-[10px] p-1.5 truncate">
                                        {{ $doc->caption }}
                                    </div>
                                    @endif
                                </div>
                            ...
                            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                <div class="space-y-3">
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-700 mb-1">Upload Foto/Video</label>
                                        <input wire:model="docFiles" type="file" multiple class="block w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20">
                                        @error('docFiles.*') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-700 mb-1">Caption (Opsional)</label>
                                        <input wire:model="docCaption" type="text" class="block w-full rounded-lg border-gray-300 py-2 px-3 text-sm focus:ring-primary focus:border-primary" placeholder="Keterangan dokumentasi...">
                                        @error('docCaption') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                                    </div>
                                    <button wire:click="saveOrderDocumentation" wire:loading.attr="disabled" class="w-full py-2.5 bg-primary text-white rounded-lg text-xs font-semibold hover:bg-primary-hover disabled:opacity-50 transition-colors shadow-sm shadow-primary/30">
                                        <span wire:loading.remove wire:target="saveOrderDocumentation, docFiles">Upload Dokumentasi</span>
                                        <span wire:loading wire:target="saveOrderDocumentation, docFiles"><i class="fa-solid fa-circle-notch fa-spin mr-1"></i> Mengupload...</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse border-t border-gray-100 rounded-b-2xl gap-2">
                    @if($selectedOrder->payment && $selectedOrder->payment->payment_type === 'bank_transfer' && $selectedOrder->status === 'pending')
                    ...
                    @endif
                    <button wire:click="closeOrderModal" type="button" class="w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-5 py-2.5 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:w-auto sm:text-sm transition-all">
                        Tutup
                    </button>
                </div>
                @endif
     ...
     <!-- Saving Detail Modal -->
    <div x-data="{ show: $wire.entangle('isSavingModalOpen') }"
         x-show="show"
         x-transition:enter="transition ease-out duration-300"
         class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full border border-gray-100">
                ...
                <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <button type="button" wire:click="closeSavingModal" class="text-gray-400 hover:text-gray-500"><i class="fa-solid fa-xmark text-xl"></i></button>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-500">Target: {{ ucfirst(str_replace('-', ' ', $selectedSaving->target_animal_type)) }} ({{ $selectedSaving->target_hijri_year }} H)</p>
                            </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
                            <div class="flex justify-between text-sm mb-1">
                                ...
                                <span class="font-bold text-primary">Rp {{ number_format($selectedSaving->saved_amount, 0, ',', '.') }}</span>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2.5 mb-1">
                                <div class="bg-primary h-2.5 rounded-full transition-all duration-500" style="width: {{ $selectedSaving->progress }}%"></div>
                            </div>
                            <div class="flex justify-between text-xs text-gray-500">
                                <span>{{ $selectedSaving->progress }}%</span>
                                <span>Target: Rp {{ number_format($selectedSaving->target_amount, 0, ',', '.') }}</span>
                            </div>
                        </div>
                                ...
                                <table class="min-w-full divide-y divide-gray-100">
                                    <thead class="bg-gray-50 sticky top-0">
                                        <tr>
                                            ...
                                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500">Jumlah</th>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-100">
                                        ...
                                        <tr>
                                            <td class="px-4 py-2.5 text-xs text-gray-500">{{ $deposit->created_at->format('d M Y') }}</td>
                                            <td class="px-4 py-2.5 text-xs font-medium">Rp {{ number_format($deposit->amount, 0, ',', '.') }}</td>
                                            <td class="px-4 py-2.5 text-xs text-gray-600 capitalize">{{ str_replace('_', ' ', $deposit->payment_method ?? '-') }}</td>
                                            <td class="px-4 py-2.5 text-xs">
                                                <span class="px-1.5 py-0.5 rounded-md text-[10px] uppercase font-bold {{ $deposit->status == 'paid' ? 'bg-green-100 text-green-700' : ($deposit->status == 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">{{ $deposit->status }}</span>
                                            </td>
                                            <td class="px-4 py-2.5 text-xs text-right">
                                                ...
                                                <button wire:click="confirmDepositPayment({{ $deposit->id }})"
                                                ...
                                                <span class="text-green-600 text-[11px] font-medium"><i class="fa-solid fa-check-circle mr-1"></i>Terkonfirmasi</span>
                                                @else
                        ...
                        <div class="border-t border-gray-100 pt-4">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                                ...
                                <div class="relative group rounded-lg overflow-hidden bg-gray-100 aspect-video border border-gray-200">
                                    ...
                                    <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center z-10">
                                        <button wire:click="deleteDocumentation({{ $doc->id }}, 'saving')" 
                                                wire:confirm="Hapus dokumentasi ini?"
                                                class="p-2 bg-red-600 text-white rounded-full hover:bg-red-700 shadow-sm transform hover:scale-110 transition-all">
                                            <i class="fa-solid fa-trash text-xs"></i>
                                        </button>
                            ...
                            <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                <div class="space-y-3">
                                    ...
                                    <button wire:click="saveSavingDocumentation" wire:loading.attr="disabled" class="w-full py-2.5 bg-primary text-white rounded-lg text-xs font-semibold hover:bg-primary-hover disabled:opacity-50 transition-colors shadow-sm shadow-primary/30">
                                        <span wire:loading.remove wire:target="saveSavingDocumentation, docFiles">Upload Dokumentasi</span>
                                        <span wire:loading wire:target="saveSavingDocumentation, docFiles"><i class="fa-solid fa-circle-notch fa-spin mr-1"></i> Mengupload...</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                Export Data {{ $activeTab === 'orders' ? 'Pesanan' : 'Tabungan' }} Qurban
                            </h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-4">
                                    Pilih rentang tanggal untuk export data {{ $activeTab === 'orders' ? 'pesanan' : 'tabungan' }}.
                                </p>
