...
<div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <select wire:model.live="roleFilter"
                        class="w-full rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-primary focus:ring focus:ring-primary/20 cursor-pointer py-2.5 px-4 text-base">
                    ...
                    <button wire:click="createUser"
                        class="bg-primary hover:bg-primary-hover text-white font-semibold py-2.5 px-4 rounded-xl inline-flex items-center justify-center transition-all duration-200 shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:-translate-y-0.5 whitespace-nowrap">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse($users as $user)
                                <tr class="hover:bg-orange-50/30 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <div
                                                    class="h-10 w-10 rounded-full bg-primary text-white flex items-center justify-center text-sm font-bold border border-gray-100 shadow-sm overflow-hidden">
                                                    @if ($user->avatar)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full {{ $user->role === 'admin' ? 'bg-purple-50 text-purple-700 border border-purple-100' : 'bg-gray-50 text-gray-600 border border-gray-100' }}">
                                            {{ ucfirst($user->role) }}
                                        </span>
                                    </td>
                                        ...
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            @if (auth()->id() !== $user->id)
                                                <button wire:click="deleteUser({{ $user->id }})"
                                                    onclick="return confirm('Yakin hapus user ini?') || event.stopImmediatePropagation()"
    ...
    <!-- Form Modal -->
    <div x-data="{ show: $wire.entangle('isFormOpen') }" x-show="show" x-transition:enter="transition ease-out duration-300"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <form wire:submit.prevent="saveUser">
                    <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="text-xl font-bold text-gray-900">
                                {{ $userId ? 'Edit Pengguna' : 'Tambah Pengguna' }}</h3>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input wire:model="name" type="text"
                                    class="w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-sm bg-gray-50 focus:bg-white transition-colors"
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-1">No. Telepon <span
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Password
                                            {{ $userId ? '(Opsional)' : '' }}</label>
                                        <input wire:model="password" type="password"
                                            class="w-full rounded-xl border-gray-300 focus:border-primary focus:ring focus:ring-primary/20 py-2.5 px-4 text-sm bg-gray-50 focus:bg-white transition-colors"
                                            placeholder="{{ $userId ? 'Kosongkan jika tidak ingin mengubah' : 'Minimal 8 karakter' }}">
                                        @error('password')
                                            <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Konfirmasi
                                            Password</label>
                    ...
                    <div
                        class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse border-t border-gray-100 rounded-b-2xl gap-3">
                        <button type="submit"
                        ...
                        <button type="button" wire:click="closeFormModal"
                            class="mt-3 w-full inline-flex justify-center rounded-xl border border-gray-300 shadow-sm px-5 py-2.5 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary sm:mt-0 sm:w-auto sm:text-sm transition-all">
                            Batal
                        </button>
                    </div>
                </form>
                ...
                @if ($selectedUser)
                    <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                        ...
                        <div class="flex items-center mb-8">
                            <div
                                class="h-20 w-20 rounded-full bg-primary text-white flex items-center justify-center text-xl font-bold border-2 border-white shadow-md overflow-hidden mr-5">
                                @if ($selectedUser->avatar)
                                    <img src="{{ Storage::url($selectedUser->avatar) }}"
                                        alt="{{ $selectedUser->initials }}" class="w-full h-full object-cover">
                                @else
                            ...
                            <div>
                                ...
                                <span
                                    class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full {{ $selectedUser->role === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800' }}">
                        ...
                        <div class="bg-gray-50 rounded-xl p-5 border border-gray-100 mb-6">
                            <div class="grid grid-cols-2 gap-6 text-sm">
                                ...
                                <div>
                                    ...
                                    <p
                                        class="font-medium {{ $selectedUser->email_verified_at ? 'text-green-600' : 'text-yellow-600' }}">
                            ...
                            <div class="border border-gray-100 rounded-xl overflow-hidden">
                                <table class="min-w-full divide-y divide-gray-100">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500">Tanggal
                                            </th>
                                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500">Tipe
                                            </th>
                                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500">Nominal
                                            </th>
                                            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500">Status
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-100">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="4"
                                                    class="px-4 py-4 text-center text-xs text-gray-400 italic">Belum
