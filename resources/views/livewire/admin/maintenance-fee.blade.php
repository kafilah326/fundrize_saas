...
<div class="space-y-6">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-3">
                                    <button wire:click="showDetail({{ $monthData['month_num'] }})"
                                        class="text-gray-500 hover:text-primary transition-colors">
                                        Detail
                                    </button>

                                    @if (
                                        !in_array($monthData['status'], ['paid', 'unverified']) &&
                                            $monthData['fee_maintenance'] > 0 &&
                                            !$monthData['is_current_month']
                                    )
                                        <button wire:click="payOnline({{ $monthData['month_num'] }})"
                                            class="text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1 rounded-lg text-xs transition-colors shadow-sm flex items-center gap-1">
                                    ...
                                    @if (in_array($monthData['status'], ['paid', 'unverified']) && optional($monthData['record'])->proof_of_payment)
                                        <a href="{{ Storage::url($monthData['record']->proof_of_payment) }}"
                                            target="_blank"
    ...
    <!-- Detail Modal -->
    <div x-data="{ show: false }" x-on:open-modal.window="if ($event.detail.name === 'detail-modal') show = true"
        x-on:close-modal.window="if ($event.detail.name === 'detail-modal') show = false" x-show="show"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-100">
                <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xl font-bold text-gray-900">
                            Detail Transaksi - Bulan
                            {{ Carbon\Carbon::createFromDate($year, $month ?? 1, 1)->translatedFormat('F Y') }}
                        </h3>
                    ...
                    <div class="overflow-y-auto max-h-[500px] border border-gray-100 rounded-xl">
                        <table class="min-w-full divide-y divide-gray-100">
                            <thead class="bg-gray-50 sticky top-0">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">ID Trx
                                    </th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Title
                                    </th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Tipe
                                    </th>
                                    <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase">
                                        Amount</th>
                                    <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Fee
                                    </th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-100">
                                @forelse($detailTransactions as $trx)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-4 py-3 whitespace-nowrap text-xs font-mono text-gray-500">
                                            {{ $trx['id_trx'] }}</td>
                                        <td class="px-4 py-3 text-xs text-gray-900">{{ Str::limit($trx['title'], 30) }}
                                        </td>
                                        <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-500">
                                            {{ $trx['type'] }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-xs text-right font-medium">Rp
                                            {{ number_format($trx['amount'], 0, ',', '.') }}</td>
                                        <td class="px-4 py-3 whitespace-nowrap text-xs text-right text-primary">Rp
                                            {{ number_format($trx['fee'], 0, ',', '.') }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-4 py-8 text-center text-gray-500 text-sm">Tidak ada
                                            transaksi berhasil pada bulan ini.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <!-- Payment Modal -->
    <div x-data="{ show: false }" x-on:open-modal.window="if ($event.detail.name === 'payment-modal') show = true"
        x-on:close-modal.window="if ($event.detail.name === 'payment-modal') show = false" x-show="show"
        x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
        class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-medium">Rp
                                    {{ number_format(collect($detailTransactions)->sum('amount'), 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                            ...
                            <div class="border-t border-gray-200 pt-2 flex justify-between items-center">
                                ...
                                <span class="font-bold text-xl text-primary">Rp
                                    {{ number_format($selectedMonthFee, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6">
                        ...
                        <div class="space-y-2 text-sm">
                            <div
                                class="flex justify-between items-center border-b border-blue-100 pb-2 last:border-0 last:pb-0">
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="font-bold text-blue-900 font-mono text-base select-all"
                                        id="account-number">{{ $bankDetails['account_number'] }}</span>
                                    <button
                                        @click="navigator.clipboard.writeText('{{ $bankDetails['account_number'] }}'); alert('Nomor rekening disalin!')"
                                        class="text-blue-400 hover:text-blue-600 transition-colors" title="Salin"
                                        type="button">
                            ...
                            <div
                                class="flex justify-between items-start border-b border-blue-100 pb-2 last:border-0 last:pb-0">
                                <span class="text-blue-600 whitespace-nowrap mr-4">Atas Nama</span>
                                <span
                                    class="font-bold text-blue-900 text-right">{{ $bankDetails['account_name'] }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <form wire:submit.prevent="pay">
                        ...
                        <button type="submit"
                            class="w-full bg-primary hover:bg-primary-hover text-white font-bold py-3 px-4 rounded-xl transition-colors shadow-lg shadow-primary/30">
                            <span wire:loading.remove wire:target="pay">Konfirmasi Pembayaran</span>
                            <span wire:loading wire:target="pay"><i class="fa-solid fa-circle-notch fa-spin"></i>
                                Processing...</span>
                        </button>
                    </form>
                </div>
                <!-- Duitku Pop Script -->
                <script
                    src="{{ config('duitku.sandbox') ? 'https://app-sandbox.duitku.com/lib/js/duitku.js' : 'https://app-prod.duitku.com/lib/js/duitku.js' }}">
                </script>
                <script>
                    window.addEventListener('open-duitku-pop', event => {
                        ...
                        checkout.process(data.reference, {
                            successEvent: function(result) {
                                window.location.href = data.returnUrl + '?status=success&reference=' + result
                                    .reference;
                            },
                            pendingEvent: function(result) {
                                window.location.href = data.returnUrl + '?status=pending&reference=' + result
                                    .reference;
                            },
