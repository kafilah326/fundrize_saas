<div x-data="{
    ...
    <section id="filter-compact" class="bg-white px-4 py-3 border-b border-gray-100">
        <div class="flex items-center gap-2">
            ...
            <div class="flex-1 flex items-center gap-2 overflow-x-auto hide-scrollbar">
                <span x-show="filterCount > 0" x-text="filterSummary" class="text-sm text-gray-600 whitespace-nowrap"></span>
            </div>
            
            <button x-show="filterCount > 0" @click="resetFilter()" class="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full flex-shrink-0">
    ...
    <div x-show="showFilter" class="fixed inset-0 z-50 flex items-end justify-center" style="display: none;">
        <!-- Backdrop -->
        <div @click="showFilter = false" x-transition:enter="transition opacity duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition opacity duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/50"></div>
        
        <!-- Sheet -->
        <div x-transition:enter="transition transform duration-300" x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0" x-transition:leave="transition transform duration-300" x-transition:leave-start="translate-y-0" x-transition:leave-end="translate-y-full" class="bg-white w-full max-w-[460px] rounded-t-3xl max-h-[85vh] overflow-hidden flex flex-col relative z-10">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button @click="toggleAkad('semua')" 
                            :class="isAkadSelected('semua') ? 'border-primary bg-orange-50 text-primary' : 'border-gray-200 text-gray-600'"
                        ...
                        @foreach($akads as $akad)
                            <button @click="toggleAkad('{{ $akad->slug }}')" 
                                :class="isAkadSelected('{{ $akad->slug }}') ? 'border-primary bg-orange-50 text-primary' : 'border-gray-200 text-gray-600'"
                                class="px-4 py-2 rounded-full text-sm font-medium border-2 transition-colors">
                                {{ $akad->name }}
                            </button>
                        @endforeach
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button @click="toggleKategori('semua')"
                            :class="isKategoriSelected('semua') ? 'border-primary bg-orange-50 text-primary' : 'border-gray-200 text-gray-600'"
                            class="px-4 py-2 rounded-full text-sm font-medium border-2 transition-colors">
                            Semua
                        </button>
                         @foreach($categories as $cat)
                            <button @click="toggleKategori('{{ $cat->slug }}')"
                                :class="isKategoriSelected('{{ $cat->slug }}') ? 'border-primary bg-orange-50 text-primary' : 'border-gray-200 text-gray-600'"
                                class="px-4 py-2 rounded-full text-sm font-medium border-2 transition-colors">
                                {{ $cat->name }}
                            </button>
                        @endforeach
            ...
            <div class="sticky bottom-0 bg-white border-t border-gray-100 px-4 py-3 flex gap-3">
                <button @click="resetFilter()" class="flex-1 py-3 bg-gray-100 text-dark rounded-xl text-sm font-semibold">Reset</button>
                <button @click="applyFilter()" class="flex-1 py-3 bg-primary text-white rounded-xl text-sm font-semibold">Terapkan</button>
            </div>
        </div>
    </div>
    ...
    <main id="main-content" class="pb-20">
        <section id="program-list" class="bg-white px-4 py-4 mt-2">
            <div class="flex items-center justify-between mb-4">
                <p class="text-xs text-gray-500">Menampilkan <span class="font-semibold text-dark">{{ $totalPrograms }} program</span></p>
            </div>
            <div class="space-y-3">
                    ...
                    <div class="flex-1 flex flex-col justify-between py-1">
                        ...
                        <div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <button class="text-gray-400"><i class="fa-solid fa-ellipsis-vertical text-xs"></i></button>
                            </div>
                            @else
            ...
            <button wire:click="loadMore" class="w-full mt-4 py-3 bg-gray-100 text-dark rounded-xl text-sm font-semibold">
                Muat Lebih Banyak
            </button>
            @endif
        </section>
