<div x-data>
    <header id="search-header" class="bg-white shadow-sm sticky top-0 z-50">
        <div class="px-4 py-3 flex items-center gap-3">
            <a href="{{ url()->previous() == url()->current() ? route('home') : url()->previous() }}" wire:navigate
            ...
            <div class="flex-1 relative">
                <input type="text" wire:model.live.debounce.300ms="query" placeholder="Cari program donasi..."
                ...
                @if (!empty($query))
                    <button wire:click="$set('query', '')"
                        class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 w-5 h-5 flex items-center justify-center">
                ...
                <div class="space-y-3">
                    @foreach ($urgentPrograms as $program)
                        <a href="{{ route('program.detail', $program['slug']) }}" wire:navigate
                            class="flex gap-3 bg-white rounded-xl border border-gray-100 p-2 hover:border-primary/30 transition-colors block">
                            ...
                            <div class="flex-1 flex flex-col justify-between py-1">
                                <div>
                                    <span
                                        class="inline-block px-2 py-0.5 bg-red-50 text-red-600 text-[10px] font-bold rounded mb-1.5">Sisa
                                        {{ $program['days_left'] }} Hari</span>
                                    <h3 class="text-sm font-bold text-dark leading-snug line-clamp-2 mb-2">
                                        {{ $program['title'] }}</h3>
                                </div>
                                <div>
                                    ...
                                    <div class="flex justify-between items-end">
                                        <div>
                                            ...
                                            <p class="text-xs font-bold text-primary">Rp
                                                {{ number_format($program['collected'] / 1000000, 1, ',', '.') }}jt</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    @foreach ($featuredPrograms as $program)
                        <a href="{{ route('program.detail', $program['slug']) }}" wire:navigate
                            ...
                            <div class="flex-1 flex flex-col justify-between py-1">
                                <div>
                                    <span
                                        class="inline-block px-2 py-0.5 bg-blue-50 text-blue-600 text-[10px] font-bold rounded mb-1.5">{{ $program['donor_count'] }}
                                        Donatur</span>
                                    <h3 class="text-sm font-bold text-dark leading-snug line-clamp-2 mb-2">
                                        {{ $program['title'] }}</h3>
                                </div>
                    ...
                    @foreach ($results as $program)
                        <a href="{{ route('program.detail', $program['slug']) }}" wire:navigate
                            ...
                            <div class="flex-1 flex flex-col justify-between py-1">
                                <div>
                                    <span
                                        class="inline-block px-2 py-0.5 bg-orange-50 text-primary text-[10px] font-bold rounded mb-1.5">{{ $program['category'] }}</span>
                                    <h3 class="text-sm font-bold text-dark leading-snug line-clamp-2 mb-2">
                                        {{ $program['title'] }}</h3>
                                </div>
                                <div>
