            ...
            <div class="grid grid-cols-1 gap-3 mb-4">
                <div class="bg-primary rounded-2xl p-5 shadow-lg">
                    <div class="flex items-center gap-3 mb-3">
                        <div>
                            ...
                            <h2 class="text-white text-2xl font-bold">Rp
                                {{ number_format($availableBalance, 0, ',', '.') }}</h2>
                        </div>
                    </div>
                    <a href="{{ route('fundraiser.withdrawal') }}" wire:navigate
                        class="w-full bg-white/20 backdrop-blur-sm text-white font-semibold py-3 rounded-xl border border-white/30 active:scale-95 transition-transform flex items-center justify-center gap-2">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                        ...
                        <h4 class="text-dark text-xl font-bold">Rp {{ number_format($withdrawnUjroh, 0, ',', '.') }}
                        </h4>
                ...
                @forelse($commissions as $commission)
                    ...
                    @php
                        ...
                        if ($commission->commissionable_type === \App\Models\Donation::class) {
                            ...
                            $donorName = $commission->commissionable->donor_name ?? 'Hamba Allah';
                            $baseAmount = $commission->commissionable->amount;
                        } elseif ($commission->commissionable_type === \App\Models\ZakatTransaction::class) {
                            $title = $commission->commissionable->zakat_type == 'maal' ? 'Zakat Mal' : 'Zakat Fitrah';
                            $donorName = $commission->commissionable->donor_name ?? 'Hamba Allah';
                            $baseAmount = $commission->commissionable->amount;
...
if ($baseAmount > 0) {
    ...
    if ($percentage > 0 && $percentage < 100) {
        $commissionText = 'Rp ' . number_format($baseAmount, 0, ',', '.');
    } else {
                        ...
                        <div class="flex items-start justify-between mb-2">
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-2">
                                    <span
                                        class="text-[10px] text-gray-400">{{ $commission->created_at->format('d M Y, H:i') }}</span>
                                    <span
                                        class="bg-emerald-50 text-emerald-600 text-[10px] font-medium px-2 py-0.5 rounded-full">Berhasil</span>
                                </div>
                            </div>
                            <div class="text-right ml-3 flex-shrink-0">
                                <p class="text-primary font-bold text-base">+Rp
                                    {{ number_format($commission->amount, 0, ',', '.') }}</p>
