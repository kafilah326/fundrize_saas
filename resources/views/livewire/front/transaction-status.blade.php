<div>
    ...
    <main id="main-content" class="pb-24">
        ...
        <div class="px-4 py-8 text-center">
            ...
            <a href="{{ route('home') }}" class="inline-block px-6 py-2 bg-primary text-white rounded-lg text-sm font-semibold">Kembali ke Beranda</a>
        </div>
        @else
        ...
        <section id="status-banner" class="bg-white px-4 py-6 text-center border-b border-gray-100">
            ...
            <p class="text-sm font-semibold text-orange-500 mt-1">{{ \Carbon\Carbon::parse($expiryTime)->translatedFormat('d F Y, H:i') }} WIB</p>
            @endif
        </section>
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <button x-data @click="navigator.clipboard.writeText('{{ $bankAccount->account_number }}'); alert('Tersalin!')" class="text-primary text-xs font-semibold hover:underline">Salin</button>
                        </div>
                    ...
                    <div class="bg-blue-50 p-3 rounded-lg mb-3">
                        <div class="flex justify-between text-xs mb-1">
                            ...
                            <span class="font-semibold text-dark">Rp {{ number_format($payment->amount, 0, ',', '.') }}</span>
                        </div>
                        @if($uniqueCode > 0)
                        <div class="flex justify-between text-xs mb-1">
            ...
            @elseif($paymentGroup === 'xendit')
                <!-- Xendit Pending -->
                 <div class="bg-gray-50 rounded-xl p-4 border border-gray-200 text-center">
                    ...
                    <a href="{{ $payment->xendit_invoice_url }}" class="inline-block px-6 py-2 bg-primary text-white rounded-lg text-sm font-semibold">Lanjut ke Pembayaran</a>
                    @else
        ...
        <section id="transaction-detail" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold text-dark">{{ $payment->created_at->translatedFormat('d M Y, H:i') }}</span>
                </div>
                <div class="flex justify-between text-sm">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold text-dark">Rp {{ number_format($payment->admin_fee, 0, ',', '.') }}</span>
                </div>
                @endif
                @if($uniqueCode > 0)
                <div class="flex justify-between text-sm">
    ...
    <div class="fixed bottom-0 left-0 right-0 max-w-[460px] mx-auto bg-white border-t border-gray-200 px-4 py-4 z-50 flex gap-3">
        <a href="{{ route('home') }}" class="flex-1 py-3 bg-white border border-gray-300 text-dark rounded-xl text-sm font-semibold text-center hover:bg-gray-50">
            Beranda
        </a>
        <a href="https://wa.me/{{ preg_replace('/^0/', '62', \App\Models\FoundationSetting::value('phone') ?? '') }}" target="_blank" class="flex-1 py-3 bg-green-500 text-white rounded-xl text-sm font-semibold text-center hover:bg-green-600 flex items-center justify-center gap-2">
