...
<div>
    ...
    <main class=" pb-20">
        ...
        <section id="hero-slider" class="relative w-full aspect-video overflow-hidden" x-data="{
            ...
            <div x-ref="carousel" class="carousel-container flex overflow-x-auto snap-x snap-mandatory h-full"
                @scroll="currentIndex = Math.round($event.target.scrollLeft / $event.target.clientWidth)">
                @foreach ($banners as $banner)
                    <div class="carousel-item min-w-full h-full relative">
                        <img class="w-full h-full object-cover" src="{{ $banner->image }}" alt="{{ $banner->title }}">
                        <div class="absolute inset-0 bg-gradient-to-t from-dark via-dark/60 to-transparent"></div>
                        <div class="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8">
                            <div class="w-full">
                                @if ($banner->badge_text)
                                    <span
                                        class="inline-block bg-primary text-white text-xs font-semibold px-3 py-1 rounded-full mb-2">{{ $banner->badge_text }}</span>
                                @endif
                                <h2 class="text-white font-bold text-xl sm:text-2xl md:text-3xl mb-2">
                                    {{ $banner->title }}</h2>
                                ...
                                @if ($banner->link)
                                    <a href="{{ $banner->link }}"
                                        class="inline-block bg-primary hover:bg-primary text-white font-semibold px-6 py-2.5 rounded-lg text-sm transition">
