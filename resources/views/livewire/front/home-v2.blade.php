    ...
    <header id="header" class="sticky top-0 bg-white shadow-sm z-50">
        <div class="flex items-center justify-between px-4 py-3 w-full">
            <div class="flex items-center gap-3">
                <img src="{{ Str::startsWith($foundation->logo, 'http') ? $foundation->logo : Storage::url($foundation->logo) }}" alt="Logo" class="w-36 h-auto object-contain">
            </div>
        ...
        <section id="hero-slider" class="relative w-full aspect-video overflow-hidden" x-data="{
            ...
            <div x-ref="carousel" class="carousel-container flex overflow-x-auto snap-x snap-mandatory h-full" @scroll="currentIndex = Math.round($event.target.scrollLeft / $event.target.clientWidth)">
                ...
                <div class="carousel-item min-w-full h-full relative">
                    <img class="w-full h-full object-cover" src="{{ Str::startsWith($banner->image, 'http') ? $banner->image : Storage::url($banner->image) }}" alt="{{ $banner->title }}">
                    ...
                    <div
                        ...
                        <a href="{{ route('program.detail', $program->slug) }}" class="block">
                            <div class="h-40 overflow-hidden relative">
                                <img class="w-full h-full object-cover" src="{{ Str::startsWith($program->image, 'http') ? $program->image : Storage::url($program->image) }}"
                                    alt="{{ $program->title }}">
                                @if ($program->category)
                                    <span
                        ...
                        <div class="flex-1 p-3 flex flex-col justify-between">
                            ...
                            <div class="mt-auto">
                                <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden mb-1.5">
                                    <div class="progress-bar bg-gradient-to-r from-teal-500 to-teal-400 h-full rounded-full"
                                        style="width: {{ $program->progress_percentage }}%"></div>
                                </div>
                ...
                <div class="flex items-start space-x-3 mb-4">
                    <div
                        ...
                        @if ($foundation->favicon)
                            <img src="{{ Str::startsWith($foundation->favicon, 'http') ? $foundation->favicon : Storage::url($foundation->favicon) }}" alt="Logo" class="w-full object-contain">
                        @else
