...
<div>
    ...
    <header id="header" class="fixed top-0 left-0 right-0 bg-white shadow-sm z-50">
        ...
        <div class="flex items-center justify-between px-4 py-3 max-w-7xl mx-auto">
            <div class="flex items-center space-x-2">
                <div class="w-10 h-10 bg-gradient-to-br from-teal-600 to-teal-500 rounded-lg flex items-center justify-center">
                ...
                <div>
                    <h1 class="text-navy-900 font-bold text-base leading-tight">{{ $foundation->name }}</h1>
                ...
                <div class="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8">
                    <div class="max-w-7xl mx-auto">
                        @if($banner->badge_text)
                            <span class="inline-block bg-teal-500 text-white text-xs font-semibold px-3 py-1 rounded-full mb-2">{{ $banner->badge_text }}</span>
                        @endif
                        ...
                        @endif
                        @if($banner->link)
                        <a href="{{ $banner->link }}" class="inline-block bg-teal-500 hover:bg-teal-600 text-white font-semibold px-6 py-2.5 rounded-lg text-sm transition">
    ...
    <section id="pilih-akad" class="px-4 py-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <a href="{{ route('program.index', ['akad' => $akad->slug]) }}" class="bg-white border-2 border-slate-200 hover:border-teal-500 rounded-xl p-4 transition flex flex-col items-center space-y-2 min-h-[110px]">
                ...
                <span class="text-navy-900 font-semibold text-sm">{{ $akad->name }}</span>
            </a>
            @endforeach
        </div>
    </section>

    <section id="program-unggulan" class="px-4 py-6 max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-navy-900 font-bold text-lg">Program Unggulan</h3>
            <a href="{{ route('program.index') }}" class="text-teal-600 text-sm font-semibold hover:text-teal-700">Lihat Semua</a>
        </div>
        <div class="flex overflow-x-auto space-x-4 pb-2 -mx-4 px-4 carousel-container">
            ...
            <a href="{{ route('program.detail', $program->slug) }}" class="block">
                    <div class="h-40 overflow-hidden relative">
                        ...
                        @if($program->category)
                        <span class="absolute top-3 left-3 inline-block bg-white/90 text-teal-700 text-xs font-semibold px-2 py-1 rounded backdrop-blur-sm shadow-sm">{{ $program->category->name }}</span>
                        @endif
                    </div>
                </a>
                <div class="p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="w-full bg-slate-200 rounded-full h-2 overflow-hidden">
                            <div class="progress-bar bg-gradient-to-r from-teal-600 to-teal-500 h-full rounded-full" style="width: {{ $program->progress_percentage }}%"></div>
                        </div>
                        <div class="flex justify-between mt-2">
                            <span class="text-navy-900 font-bold text-sm">Rp {{ number_format($program->collected_amount, 0, ',', '.') }}</span>
    ...
    <section id="menu-program" class="px-4 py-6 max-w-7xl mx-auto">
        <h3 class="text-navy-900 font-bold text-lg mb-4">Menu Program</h3>
        <div class="flex overflow-x-auto space-x-2 pb-2 -mx-4 px-4 carousel-container">
            <a href="{{ route('program.index') }}" class="bg-teal-500 text-white font-semibold px-4 py-2 rounded-full text-sm whitespace-nowrap shadow-sm">
                Semua Program
            </a>
    ...
    <section id="program-lainnya" class="px-4 py-6 max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-navy-900 font-bold text-lg">Program Lainnya</h3>
            <a href="{{ route('program.index') }}" class="text-slate-500 hover:text-navy-900 transition-colors">
                ...
                <div class="flex-1 p-3 flex flex-col justify-between">
                    ...
                    <div class="mt-auto">
                        <div class="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden mb-1.5">
                            <div class="progress-bar bg-gradient-to-r from-teal-500 to-teal-400 h-full rounded-full" style="width: {{ $program->progress_percentage }}%"></div>
                        </div>
                        <div class="flex justify-between items-end">
                            <div>
                                <span class="text-navy-900 font-bold text-xs">Rp {{ number_format($program->collected_amount, 0, ',', '.') }}</span>
                            </div>
        ...
        <a href="{{ route('program.index') }}" class="block text-center w-full mt-4 bg-white border border-slate-200 hover:border-teal-500 text-slate-600 hover:text-teal-600 font-semibold py-3 rounded-lg text-sm transition-colors shadow-sm">
            Muat Lebih Banyak Program
        </a>
    </section>

    <section id="transparansi" class="px-4 py-6 max-w-7xl mx-auto">
        <h3 class="text-navy-900 font-bold text-lg mb-4">Transparansi &amp; Akuntabilitas</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <a href="#" class="bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl p-4 text-white shadow-md hover:shadow-lg transition-all hover:-translate-y-1 block group">
                ...
                <p class="text-sm text-teal-50 mb-3 opacity-90">Akses laporan keuangan bulanan dan tahunan</p>
                <div class="text-xs text-teal-100 font-medium">Transparan & Diaudit</div>
            </a>
            <a href="#" class="bg-gradient-to-br from-navy-800 to-navy-900 rounded-xl p-4 text-white shadow-md hover:shadow-lg transition-all hover:-translate-y-1 block group">
                <div class="flex items-center justify-between mb-3">
    ...
    <section id="about-yayasan" class="px-4 py-6 max-w-7xl mx-auto">
        <h3 class="text-navy-900 font-bold text-lg mb-4">Tentang {{ $foundation->name }}</h3>
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
            <div class="flex items-start space-x-3 mb-4">
                <div class="w-16 h-16 bg-slate-50 rounded-xl flex items-center justify-center flex-shrink-0 border border-slate-100 overflow-hidden p-2">
                    ...
                    @else
                        <i class="fa-solid fa-hand-holding-heart text-teal-500 text-2xl"></i>
                    @endif
                </div>
                <div>
                    <h4 class="text-navy-900 font-bold text-base mb-1">{{ $foundation->name }}</h4>
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                <span class="bg-teal-50 text-teal-700 border border-teal-100 text-[10px] font-semibold px-2.5 py-1 rounded-full uppercase tracking-wide">Terdaftar & Berizin</span>
                <span class="bg-navy-50 text-navy-700 border border-navy-100 text-[10px] font-semibold px-2.5 py-1 rounded-full uppercase tracking-wide">Transparan</span>
            </div>
        </div>
    </section>

    <section id="cta-banner" class="px-4 py-6 max-w-7xl mx-auto mb-4">
        <div class="bg-gradient-to-r from-teal-600 to-teal-500 rounded-2xl p-8 text-white text-center shadow-lg relative overflow-hidden">
            <!-- Decorative circles -->
            ...
            <div class="relative z-10">
                ...
                <p class="text-teal-50 text-sm mb-6 max-w-md mx-auto opacity-90">Setiap donasi Anda membawa perubahan nyata bagi mereka yang membutuhkan</p>
                <a href="{{ route('program.index') }}" class="inline-block bg-white text-teal-600 hover:bg-slate-50 font-bold px-8 py-3.5 rounded-xl text-sm transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5">
