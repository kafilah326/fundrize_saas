<div>
        ...
        <section id="status-banner" class="bg-white px-4 py-6 text-center border-b border-gray-100">
            ...
            @else
                ...
                <p class="text-sm font-semibold text-primary mt-1">
                    {{ \Carbon\Carbon::parse($expiryTime)->format('d M Y, H:i') }} WIB</p>
            @endif
        </section>
        ...
        @if ($paymentStatus === 'pending')
            <section id="payment-instruction" class="bg-white px-4 py-6 mt-2">
                    ...
                    <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <button x-data
                                    @click="navigator.clipboard.writeText('{{ $bankAccount->account_number }}'); $el.innerText = 'Tersalin!'"
                            ...
                            <div class="bg-blue-50 p-3 rounded-lg mb-3">
                                <div class="flex justify-between text-xs mb-1">
                                    ...
                                    <span class="font-semibold text-dark">Rp
                                        {{ number_format($checkout['amount'] + $checkout['admin_fee'], 0, ',', '.') }}</span>
                                </div>
                        ...
                        <div class="bg-orange-50 p-3 rounded-lg flex gap-2">
                            ...
                            <p class="text-xs text-gray-600">PENTING: Mohon transfer TEPAT hingga 3 digit terakhir (kode
                                unik) agar pembayaran terverifikasi otomatis.</p>
                        </div>
                    </div>
                @elseif($paymentGroup === 'xendit')
                    ...
                    <div class="bg-gray-50 rounded-xl p-4 border border-gray-200 text-center">
                        <p class="text-sm text-dark mb-3">Silakan selesaikan pembayaran melalui halaman pembayaran
                            online.</p>
                        <a href="{{ \App\Models\Payment::where('external_id', $trxId)->value('xendit_invoice_url') }}"
                ...
                <div class="mt-4">
                    ...
                    <p class="text-2xl font-bold text-primary text-center">Rp
                        {{ number_format($totalTransfer, 0, ',', '.') }}</p>
                </div>
            </section>
        @endif
        ...
        <section id="transaction-detail" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    <span
                        class="font-semibold text-dark text-right max-w-[60%] line-clamp-1">{{ $checkout['program_name'] ?? ($checkout['target_name'] ?? 'Donasi Program') }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Nominal</span>
                    <span class="font-semibold text-dark">Rp
                        {{ number_format($checkout['amount'], 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Biaya Admin</span>
                    <span class="font-semibold text-dark">Rp
                        {{ number_format($checkout['admin_fee'], 0, ',', '.') }}</span>
                </div>
                @if ($uniqueCode > 0)
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-600">Kode Unik</span>
    ...
    <div
        ...
        <a href="{{ route('my-donation.index') }}" wire:navigate
        ...
        <a href="{{ route('home') }}" wire:navigate
            class="flex-1 py-3 bg-primary text-white rounded-xl text-sm font-semibold text-center hover:bg-primary/90">
            Selesai
        </a>
    </div>
