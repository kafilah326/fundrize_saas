<div>
    <x-page-header title="Status Pembayaran" :showBack="false" />
    ...
    <main id="main-content" class="pb-24">
        ...
        <section id="status-banner" class="bg-white px-4 py-6 text-center border-b border-gray-100">
            ...
            <p class="text-sm font-semibold text-primary mt-1">{{ \Carbon\Carbon::parse($expiryTime)->format('d M Y, H:i') }} WIB</p>
            @endif
        </section>
            ...
            @if($paymentGroup === 'bank_transfer' && $bankAccount)
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <div class="flex items-center justify-between mb-4">
                        <span class="text-sm font-semibold text-gray-600">Bank {{ strtoupper($bankAccount->bank_name) }}</span>
                        @if($bankAccount->icon)
                        <img src="{{ $bankAccount->icon }}" alt="Bank Logo" class="h-6 object-contain" onerror="this.style.display='none'">
                        @else
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center justify-between">
                            <span class="text-lg font-bold text-dark tracking-wide">{{ $bankAccount->account_number }}</span>
                            <button x-data @click="navigator.clipboard.writeText('{{ $bankAccount->account_number }}'); $el.innerText = 'Tersalin!'" class="text-primary text-xs font-semibold hover:underline">Salin</button>
                        </div>
                    ...
                    <div class="bg-blue-50 p-3 rounded-lg mb-3">
                         <div class="flex justify-between text-xs mb-1">
                            ...
                            <span class="font-semibold text-dark">Rp {{ number_format($checkout['amount'] + $checkout['admin_fee'], 0, ',', '.') }}</span>
                        </div>
                    ...
                    <div class="bg-orange-50 p-3 rounded-lg flex gap-2">
                        <i class="fa-solid fa-triangle-exclamation text-primary text-xs mt-0.5"></i>
                        <p class="text-xs text-gray-600">PENTING: Mohon transfer TEPAT hingga 3 digit terakhir (kode unik) agar pembayaran terverifikasi otomatis.</p>
                    </div>
                </div>
            ...
            @elseif($paymentGroup === 'xendit')
                <!-- Xendit Pending -->
                 <div class="bg-gray-50 rounded-xl p-4 border border-gray-200 text-center">
                    ...
                    <a href="{{ \App\Models\Payment::where('external_id', $trxId)->value('xendit_invoice_url') }}" class="inline-block px-6 py-2 bg-primary text-white rounded-lg text-sm font-semibold">Lanjut ke Pembayaran</a>
                </div>
            @endif

            <div class="mt-4">
                <p class="text-xs text-gray-600 text-center">Total Pembayaran</p>
                <p class="text-2xl font-bold text-primary text-center">Rp {{ number_format($totalTransfer, 0, ',', '.') }}</p>
            </div>
        </section>
        @endif
        ...
        <section id="transaction-detail" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="space-y-3">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-semibold text-dark">{{ $trxId }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Program</span>
                    <span class="font-semibold text-dark text-right max-w-[60%] line-clamp-1">{{ $checkout['program_name'] ?? $checkout['target_name'] ?? 'Donasi Program' }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Nominal</span>
                    <span class="font-semibold text-dark">Rp {{ number_format($checkout['amount'], 0, ',', '.') }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Biaya Admin</span>
                    <span class="font-semibold text-dark">Rp {{ number_format($checkout['admin_fee'], 0, ',', '.') }}</span>
                </div>
                @if($uniqueCode > 0)
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600">Kode Unik</span>
    ...
    <!-- Bottom Actions -->
    <div class="fixed bottom-0 left-0 right-0 max-w-[460px] mx-auto bg-white border-t border-gray-200 px-4 py-4 z-50 flex gap-3">
        <a href="{{ route('my-donation.index') }}" wire:navigate class="flex-1 py-3 bg-white border border-gray-300 text-dark rounded-xl text-sm font-semibold text-center hover:bg-gray-50">
            Cek Status
        </a>
        <a href="{{ route('home') }}" wire:navigate class="flex-1 py-3 bg-primary text-white rounded-xl text-sm font-semibold text-center hover:bg-primary/90">
            Selesai
        </a>
    </div>
    ...
    @if($metaPixelEnabled && $metaPixelId)
        @if($paymentStatus === 'pending')
            {{-- InitiateCheckout: user baru submit form donasi, belum bayar --}}
            <script>
        ...
        @elseif($paymentStatus === 'paid')
            {{-- Purchase: pembayaran berhasil dikonfirmasi --}}
            <script>
                if (typeof fbq !== 'undefined') {
                    fbq('track', 'Purchase', {
                        value: {{ (float) ($checkout['amount'] ?? 0) }},
