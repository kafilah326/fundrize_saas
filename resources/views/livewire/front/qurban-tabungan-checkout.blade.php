<div>
    ...
    <main id="main-content" class="pb-52">
        ...
        <section id="donor-section" class="bg-white px-4 py-5 mb-2">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input type="text" wire:model.live="name" placeholder="Masukkan nama lengkap" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" {{ $isAnonymous ? 'disabled' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" wire:model.live="qurbanName" placeholder="Untuk penyebutan niat/sertifikat" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary">
                </div>
        ...
        <section id="deposit-section" class="bg-white px-4 py-5 mb-2">
            ...
            <div class="grid grid-cols-2 gap-2 mb-3">
                ...
                <button wire:click="setDeposit({{ $val }})" class="nominal-btn py-2.5 px-3 rounded-lg text-sm font-semibold {{ $deposit == $val && !$showCustomDeposit ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
            ...
            <div>
                ...
                <div class="flex items-center gap-2 border border-gray-300 rounded-lg px-3 focus-within:ring-2 focus-within:ring-primary/30 focus-within:border-primary transition-all">
                    <span class="text-sm text-gray-600">Rp</span>
                    <input type="number" wire:model.live="customDeposit" wire:focus="setDeposit('custom')" placeholder="0" class="flex-1 py-2.5 text-sm focus:outline-none w-full">
                </div>
            </div>

            <button wire:click="setDeposit(0)" class="text-xs text-primary mt-3 underline hover:text-primary/80">Lewati setoran pertama</button>
        </section>
        ...
        <section id="reminder-section" class="bg-white px-4 py-5">
            ...
            <div class="mt-3 transition-all duration-300">
                <select wire:model.live="reminderFrequency" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary">
    ...
    <!-- Sticky Summary -->
    <div id="sticky-summary" class="fixed bottom-0 left-0 right-0 max-w-[460px] mx-auto bg-white border-t-2 border-gray-200 p-4 z-50">
        <div class="space-y-2 mb-3">
            <div class="flex justify-between text-xs">
                ...
                @endphp
                <span class="font-semibold text-dark">{{ $selectedTarget['name'] }} – Rp {{ number_format($selectedTarget['price'], 0, ',', '.') }}</span>
            </div>
            <div class="flex justify-between text-xs">
                <span class="text-gray-600">Setoran pertama</span>
                <span class="font-semibold text-dark">{{ $deposit > 0 ? 'Rp ' . number_format($deposit, 0, ',', '.') : '-' }}</span>
            </div>
