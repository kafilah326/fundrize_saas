<div>
    ...
    <main id="main-content" class="pb-48">
        ...
        <section id="payment-methods" class="bg-white px-4 py-5 mb-2">
            ...
            <div class="space-y-3">
                <div class="mb-4">
                    <h3 class="text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wide">Transfer Bank (Cek Manual)</h3>
                    <div class="space-y-2">
                        ...
                        <label class="flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition-colors
                            ...
                            <input type="radio" wire:click="selectBank('{{ strtolower($bank->bank_name) }}')" name="payment" value="{{ strtolower($bank->bank_name) }}" class="w-4 h-4 text-primary" {{ $selectedMethod === strtolower($bank->bank_name) && $paymentGroup === 'bank_transfer' ? 'checked' : '' }}>
                            <div class="w-10 h-8 bg-white rounded flex items-center justify-center border border-gray-100 overflow-hidden">
                                @if($bank->icon)
                                <img src="{{ $bank->icon }}" alt="{{ $bank->bank_name }}" class="w-full h-full object-contain p-1">
                                @else
                                <span class="text-xs font-bold text-gray-700">{{ strtoupper(substr($bank->bank_name, 0, 3)) }}</span>
                                @endif
                            </div>
                            <div class="flex-1">
                ...
                <div class="mb-4">
                    <h3 class="text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wide">Pembayaran Otomatis</h3>
                    <div class="space-y-2">
                        <label class="flex items-center gap-3 p-3 border-2 rounded-xl cursor-pointer transition-colors
                            {{ $paymentGroup === 'xendit' ? 'border-primary bg-primary/5' : 'border-gray-200 bg-white hover:border-primary/50' }}">
                            <input type="radio" wire:click="selectXendit" name="payment" value="xendit" class="w-4 h-4 text-primary" {{ $paymentGroup === 'xendit' ? 'checked' : '' }}>
        ...
        <section id="payment-info" class="mx-4 p-3 rounded-lg mb-2 {{ $paymentGroup === 'bank_transfer' ? 'bg-orange-50 border border-orange-200' : 'bg-blue-50 border border-blue-200' }}">
            <div class="flex gap-2">
                <i class="fa-solid {{ $paymentGroup === 'bank_transfer' ? 'fa-triangle-exclamation text-orange-600' : 'fa-info-circle text-blue-600' }} text-sm mt-0.5"></i>
                <div>
                    <p class="text-xs {{ $paymentGroup === 'bank_transfer' ? 'text-orange-800' : 'text-blue-800' }} font-medium">
                        {{ $paymentGroup === 'bank_transfer' ? 'Konfirmasi Manual Diperlukan' : 'Verifikasi Otomatis' }}
                    </p>
                    <p class="text-xs {{ $paymentGroup === 'bank_transfer' ? 'text-orange-700' : 'text-blue-700' }} mt-1">
    ...
    <div id="sticky-payment" class="fixed bottom-0 left-0 right-0 max-w-[460px] mx-auto bg-white border-t-2 border-gray-200 p-4 z-50">
        <div class="space-y-2 mb-3">
            ...
            <div class="flex justify-between text-xs">
                ...
                <span class="font-semibold text-dark">{{ $adminFee === 0 ? 'Gratis' : 'Rp ' . number_format($adminFee, 0, ',', '.') }}</span>
            </div>
        ...
        <button wire:click="pay" wire:loading.attr="disabled" class="w-full bg-primary text-white py-3 rounded-xl text-sm font-semibold shadow-lg active:scale-95 transition-transform hover:bg-primary/90 flex items-center justify-center gap-2">
