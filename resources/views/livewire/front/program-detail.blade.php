<div x-data="{
    ...
    <main id="main-content" class="pb-24">
        ...
        <section id="info-section" class="bg-white px-4 py-4">
            <div class="flex flex-wrap gap-2 mb-3">
                ...
                <span class="px-3 py-1 bg-orange-50 text-primary text-xs font-semibold rounded-full">{{ $category->name }}</span>
                @endforeach
                
                @if($program->is_urgent)
                <span class="px-3 py-1 bg-red-50 text-red-700 text-xs font-semibold rounded-full flex items-center gap-1">
                    <i class="fa-solid fa-fire text-xs"></i> Mendesak
                </span>
                @endif
            </div>
            <h2 class="text-xl font-bold text-dark mb-2">{{ $program->title }}</h2>
        ...
        <section id="progress-section" class="bg-white px-4 py-4 mt-2">
            <div class="mb-3">
                <div class="flex items-end gap-2 mb-1">
                    <span class="text-2xl font-bold text-dark">Rp {{ number_format($program->collected_amount, 0, ',', '.') }}</span>
                    <span class="text-sm text-gray-500 pb-1">terkumpul</span>
                </div>
                @if($program->target_amount)
                <p class="text-sm text-gray-600">dari target Rp {{ number_format($program->target_amount, 0, ',', '.') }}</p>
                @else
            ...
            <div class="flex items-center justify-between text-sm">
                ...
                <div class="flex items-center gap-1 text-gray-600">
                    ...
                    @else
                    <span class="font-semibold text-dark flex items-center gap-1"><i class="fa-solid fa-infinity text-xs"></i> Tanpa Batas</span>
                    @endif
                </div>
            </div>
        </section>
        ...
        <section id="description-section" class="bg-white px-4 py-4 mt-2" x-data="{ expanded: false }">
            ...
            <div class="text-sm text-gray-700 leading-relaxed space-y-3">
                ...
                <button @click="expanded = !expanded" class="text-primary font-semibold text-sm flex items-center gap-1 mt-2 w-full justify-center py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
        ...
        <section id="update-section" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="relative max-h-72 overflow-hidden">
                    ...
                    <div class="flex gap-3">
                        ...
                        <div class="flex-1 pb-4">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-semibold text-primary">{{ $update->published_at->diffForHumans() }}</span>
                            </div>
                            <h4 class="text-sm font-semibold text-dark mb-2">{{ $update->title }}</h4>
                            <div class="text-sm text-gray-600 mb-2 rich-text-content">{!! $update->description !!}</div>
                        </div>
                    </div>
                    @endforeach
                </div>
                ...
                <!-- Gradient Overlay -->
                <div class="absolute bottom-0 left-0 w-full h-24 bg-gradient-to-t from-white via-white/80 to-transparent flex items-end justify-center pb-0">
                </div>
            </div>

            <button @click="showUpdateModal = true" class="text-primary font-semibold text-sm w-full py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors mt-2">
                Selengkapnya
            </button>
        </section>
        @endif
        ...
        <section id="distribution-section" class="bg-white px-4 py-4 mt-2" x-data="{ expanded: false }">
            ...
            <div class="flex items-center justify-between py-2 border-b border-gray-100">
                ...
                <span class="text-sm font-bold text-dark">Rp {{ number_format($totalDistributed, 0, ',', '.') }}</span>
            </div>
            
            <div x-show="expanded" x-collapse>
                <div class="flex items-center justify-between py-2 border-b border-gray-100">
                    <span class="text-sm text-gray-600">Sisa Dana</span>
                    <span class="text-sm font-bold text-dark">Rp {{ number_format($remaining, 0, ',', '.') }}</span>
                </div>
                ...
                <div class="space-y-3 mt-3">
                    ...
                    <div class="p-3 bg-gray-50 rounded-lg">
                        <div class="flex justify-between mb-1">
                            ...
                            <span class="text-xs text-gray-500">{{ $dist->documentation_date->format('d M Y') }}</span>
                        </div>
            ...
            <button @click="expanded = !expanded" class="text-primary font-semibold text-sm w-full py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors mt-2 flex items-center justify-center gap-1">
        ...
        <section id="donor-section" class="bg-white px-4 py-4 mt-2">
                ...
                <div class="flex items-center justify-between border-b border-gray-50 pb-2 last:border-0">
                    ...
                    <div class="text-right">
                        <p class="text-sm font-bold text-primary">
                            Rp {{ number_format($donation->amount + ($donation->payment?->unique_code ?? 0), 0, ',', '.') }}
                        </p>
                    </div>
                </div>
                @endforeach
            ...
            <button @click="showDonorModal = true" class="text-primary font-semibold text-sm w-full py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors mt-2">
                Lihat Semua
            </button>
            @endif
        </section>
         ...
         <section id="related-section" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2">
                ...
                <div class="flex-shrink-0 w-64">
                    <a href="{{ route('program.detail', $related->slug) }}" wire:navigate class="block w-full h-full bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                        <div class="aspect-video overflow-hidden h-32 w-full">
                            <img class="w-full h-full object-cover" src="{{ $related->image }}" alt="{{ $related->title }}" />
                        </div>
                        <div class="p-3">
                            <h4 class="text-sm font-semibold text-dark mb-1 line-clamp-2 h-10">{{ $related->title }}</h4>
                            <div class="mt-2">
                                <div class="flex justify-between items-end mb-1">
                                    <span class="text-xs text-gray-500">Terkumpul</span>
                                    <span class="text-xs font-bold text-primary">Rp {{ number_format($related->collected_amount, 0, ',', '.') }}</span>
                                </div>
                                <div class="w-full bg-gray-100 rounded-full h-1.5">
                                    <div class="bg-primary h-1.5 rounded-full" style="width: {{ $related->progress }}%"></div>
                                </div>
                            </div>
                        </div>
                    </a>
    ...
    <!-- CTA Button -->
    <div id="cta-button" class="fixed bottom-0 left-0 right-0 max-w-[460px] mx-auto bg-white border-t border-gray-200 px-4 py-3 z-40">
        <a href="{{ route('program.checkout', $slug) }}" wire:navigate class="block w-full py-3 bg-primary text-white rounded-xl text-base font-bold shadow-lg active:scale-95 transition-transform text-center hover:bg-primary/90">
            Donasi Sekarang
        </a>
    </div>
    ...
    <div x-show="showUpdateModal" 
        ...
        <div class="bg-white w-full max-w-[460px] max-h-[85vh] rounded-2xl overflow-hidden flex flex-col shadow-xl"
            ...
            <div class="p-4 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
                ...
                <button @click="showUpdateModal = false" class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200">
    ...
    <div x-show="showDonorModal" 
        ...
        <div class="bg-white w-full max-w-[460px] max-h-[85vh] rounded-2xl overflow-hidden flex flex-col shadow-xl"
            ...
            <div class="p-4 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
                ...
                <button @click="showDonorModal = false" class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>
            <div class="p-4 overflow-y-auto">
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between border-b border-gray-50 pb-2 last:border-0">
                        ...
                        <div class="text-right">
                            ...
                            <span class="text-[10px] text-gray-400 block">+kode unik {{ $donation->payment->unique_code }}</span>
                            @endif
                        </div>
                    </div>
                    @endforeach
