    ...
    <main id="main-content" class="pb-20">
        <section id="profile-header" class="bg-white px-4 py-6">
            <div class="flex items-center gap-4">
                <div
                    class="w-16 h-16 rounded-full overflow-hidden bg-primary text-white flex items-center justify-center text-xl font-bold border-2 border-white shadow-lg">
                    @if ($user->avatar)
                        <img src="{{ Storage::url($user->avatar) }}" alt="{{ $user->initials }}"
        ...
        <section id="history-menu" class="bg-white px-4 py-4 mt-2">
            <div class="space-y-3">
                <a href="{{ route('qurban.history') }}" wire:navigate
                    class="w-full flex items-center gap-4 p-3 rounded-xl bg-orange-50 border border-orange-100 hover:bg-orange-100 transition-colors">
                ...
                <a href="{{ route('zakat.history') }}" wire:navigate
                    class="w-full flex items-center gap-4 p-3 rounded-xl bg-orange-50 border border-orange-100 hover:bg-orange-100 transition-colors">
                    <div class="w-12 h-12 bg-primary rounded-xl flex items-center justify-center">
                        <i class="fa-solid fa-scale-balanced text-white text-lg"></i>
                    </div>
                    <div class="flex-1 text-left">
                        <h3 class="text-sm font-semibold text-dark">Zakat Saya</h3>
                        <p class="text-xs text-gray-600">Lihat riwayat pembayaran zakat Anda</p>
                    </div>
                    <i class="fa-solid fa-chevron-right text-gray-400 text-sm"></i>
                </a>
            </div>
        ...
        <section id="account-menu" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="space-y-1">
                @if (!$user->fundraiser || $user->fundraiser->status === 'rejected')
                    <a href="{{ route('fundraiser.register') }}" wire:navigate
                        class="w-full flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                ...
                @elseif($user->fundraiser->status === 'pending')
                    <div class="w-full flex items-center gap-4 p-3 rounded-lg opacity-60 cursor-not-allowed bg-gray-50">
                        ...
                        <span class="flex-1 text-left text-sm font-medium text-dark">Menunggu Konfirmasi
                            Fundriser</span>
                        <i class="fa-solid fa-lock text-gray-400 text-sm"></i>
                    </div>
                @elseif($user->fundraiser->status === 'approved')
                    <a href="{{ route('fundraiser.dashboard') }}" wire:navigate
                        class="w-full flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors">
        ...
        <section id="foundation-info" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="space-y-1">
                ...
                <a href="{{ route('foundation.legality') }}" wire:navigate
                    class="w-full flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                    <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
        ...
        <section id="help-menu" class="bg-white px-4 py-4 mt-2">
            ...
            <div class="space-y-1">
                ...
                @endphp
                <a href="https://wa.me/{{ $waNumber }}?text=Halo%20Admin%2C%20saya%20butuh%20bantuan"
                    target="_blank"
                    class="w-full flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                    <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
                ...
                <button onclick="alert('Fitur FAQ akan segera tersedia')"
                    class="w-full flex items-center gap-4 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                    <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
        ...
        <section id="system-menu" class="bg-white px-4 py-4 mt-2 mb-4">
            <div class="space-y-1">
                <button wire:click="logout"
                    class="w-full flex items-center gap-4 p-3 hover:bg-red-50 rounded-lg text-red-600 transition-colors">
