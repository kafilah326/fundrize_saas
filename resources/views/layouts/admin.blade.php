...
<body class="bg-gray-50 font-sans text-gray-800 antialiased selection:bg-primary selection:text-white"
    x-data="{ sidebarOpen: false }">

    <div class="flex h-screen overflow-hidden">
        <!-- Sidebar -->
        <aside
            class="fixed inset-y-0 left-0 z-50 w-64 bg-dark text-white transition-transform duration-300 ease-in-out transform lg:static lg:translate-x-0 shadow-xl"
            ...
            <div class="flex items-center justify-center h-20 bg-dark-lighter/50 border-b border-gray-800">
                <div class="flex items-center space-x-2">
                    @if ($foundation && $foundation->logo)
                        <img src="{{ $foundation->logo }}" alt="{{ $foundation->name }}"
                            class="h-12 w-auto object-contain brightness-0 invert">
                    @else
                        <div
                            class="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-white font-bold text-lg shadow-lg">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open"
                                ...
                                <div
                                    class="h-10 w-10 rounded-full bg-primary text-white flex items-center justify-center text-sm font-bold border-2 border-white shadow-sm group-hover:border-primary transition-all overflow-hidden">
                                    @if (Auth::user()->avatar)
                                        <img src="{{ Storage::url(Auth::user()->avatar) }}"
                                            alt="{{ Auth::user()->initials }}" class="w-full h-full object-cover">
                                    @else
                                        {{ Auth::user()->initials }}
                                    @endif
                                </div>
                                <i
                                    class="fa-solid fa-chevron-down text-xs text-gray-400 group-hover:text-primary transition-colors"></i>
                            </button>
                                        ...
                                        input.onchange = async () => {
                                            ...
                                            if (/^image\//.test(file.type)) {
                                                ...
                                                try {
                                                    ...
                                                    const range = _this.editor
                                                        .getSelection(true);
                                                    _this.editor.insertEmbed(range
                                                        .index, 'image', data
                                                        .url);
